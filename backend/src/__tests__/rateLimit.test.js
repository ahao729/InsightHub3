...
const { rateLimit, PLAN_LIMITS, PLAN_MONTHLY_LIMITS } = require('../middleware/rateLimit');
...
describe('rateLimit — per-minute limiting', () => {
  ...
  test('sets X-RateLimit-Monthly headers', () => {
    ...
    middleware(mockReq(), res, next);
    expect(res._headers['X-RateLimit-Monthly-Limit']).toBe(PLAN_MONTHLY_LIMITS.free);
